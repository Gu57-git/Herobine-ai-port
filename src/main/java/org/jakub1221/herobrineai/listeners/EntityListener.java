...
public class EntityListener implements Listener {
        ...
        if (event.getEntity() instanceof Arrow) {
            ...
            if (arrow.getShooter() instanceof Player) {
                ...
                if (player.getInventory().getItemInMainHand() != null) {
                    ...
                    if (ItemName.getLore(item) != null) {
                        if (ItemName.getLore(item).containsAll(equalsLore) && PluginCore.getConfigDB().UseArtifactBow) {
    ...
    public void onEntityTarget(EntityTargetLivingEntityEvent event) {
        if (event.getTarget() != null) {
            if (event.getTarget().getEntityId() == PluginCore.HerobrineEntityID) {
        ...
        if (event.getEntity().getEntityId() == PluginCore.HerobrineEntityID) {
            ...
            if (event instanceof EntityDamageByEntityEvent) {
                ...
                if (dEvent.getDamager() instanceof Player) {
                    ...
                    if (player.getInventory().getItemInMainHand() != null) {
                        if (player.getInventory().getItemInMainHand().getType() == Material.DIAMOND_SWORD) {
                            if (ItemName.getLore(player.getInventory().getItemInMainHand()) != null) {
                                ...
                                getLore = (ArrayList<String>) ItemName.getLore(itemInHand);
                    ...
                    if (zmb.getCustomName() != null && (zmb.getCustomName().equals("Artifact Guardian") || zmb.getCustomName().equals("Herobrine's Warrior"))) {
                        dEvent.setDamage(dEvent.getDamage() * 3);
                    }
                } else if (dEvent.getDamager() instanceof Skeleton) {
                    Skeleton zmb = (Skeleton) dEvent.getDamager();
                    if (zmb.getCustomName() != null && zmb.getCustomName().equals("Demon")) {
                        dEvent.setDamage(dEvent.getDamage() * 3);
                    }
                }
            }
        ...
        for (int i = 1; i <= 2500; i++) {
            if (PluginCore.getConfigDB().config.contains("config.Drops." + Integer.toString(i)) == true) {
                ...
                if (chance <= requiredRoll) {
                    ...
                    Material mat = Material.matchMaterial(Integer.toString(i));
                    if (mat != null) {
                        ItemStack its = new ItemStack(mat, itsAmount);
                        PluginCore.HerobrineNPC.getBukkitEntity().getLocation().getWorld().dropItemNaturally(
                            PluginCore.HerobrineNPC.getBukkitEntity().getLocation(), its);
                    }
                }
            }
        }
